...
% Written by Manuel Diaz, NTU, 04.29.2015.
    ...
    for j = 2:N-2 % for all faces the domain cells
        ...
        switch fluxMethod
            ...
            case 'SHLL' % SHLL
                flux(:,j) = SHLLflux(qL(:,j),qR(:,j),gamma);
        end
    ...
    switch fluxMethod
        ...
        case 'SHLL' % SHLL
            flux(:,1) = SHLLflux(qL(:,1),qR(:,1),gamma);
    end
    ...
    switch fluxMethod
        ...
        case 'SHLL' % SHLL
            flux(:,N-1) = SHLLflux(qL(:,N-1),qR(:,N-1),gamma);
    end
...
function SLLE = SHLLflux(qL,qR,gamma)
    % Compute Splitted HLL flux
    ...
    % Compute and correct the Mach number
    M = u/a; 
    if (M > 1), M = 1; end
    if (M <-1), M =-1; end
    M2 = M*M;
