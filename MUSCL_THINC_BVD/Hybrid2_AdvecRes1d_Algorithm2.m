function res = Hybrid2_AdvecRes1d_Algorithm2(qi,flux,dflux,S,dx,limiter)
%
...
TBV_T = min( cat(3,...
    abs(circshift(qiph_M,+1)-qimh_T)+abs(qiph_T-circshift(qimh_M,-1)),...
    abs(circshift(qiph_T,+1)-qimh_T)+abs(qiph_T-circshift(qimh_T,-1)),...
    abs(circshift(qiph_M,+1)-qimh_T)+abs(qiph_T-circshift(qimh_T,-1)),...
    abs(circshift(qiph_T,+1)-qimh_T)+abs(qiph_T-circshift(qimh_M,-1))),[],3);

TBV_M = min( cat(3,...
    abs(circshift(qiph_M,+1)-qimh_M)+abs(qiph_M-circshift(qimh_M,-1)),...
    abs(circshift(qiph_T,+1)-qimh_M)+abs(qiph_M-circshift(qimh_T,-1)),...
    abs(circshift(qiph_M,+1)-qimh_M)+abs(qiph_M-circshift(qimh_T,-1)),...
    abs(circshift(qiph_T,+1)-qimh_M)+abs(qiph_M-circshift(qimh_M,-1))),[],3);
